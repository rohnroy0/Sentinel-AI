...
- [x] **Production Audit & Database Abstraction Optimization (v1.1.0):** Implemented pluggable Database Adapter pattern (`adapter.py`, `sqlite_adapter.py`, `supabase_adapter.py`, `repository.py`), strict user isolation enforcement, explicit `AUTH_MODE` (demo vs supabase) with isolated temporary session identities, FastAPI `/health` endpoint, Pydantic response schemas, legacy script archiving, and expanded 12-test automated verification suite (`test_agent_system.py`).
- [x] **Render Memory Limit Optimization & Resource Controls (v1.1.1):** Single worker configuration (`--workers 1 --timeout 300`), 20-item Bounded LRU caches for `investigations` and `agent_investigations`, DB fallback state hydration, memoized local CVE database cache (`_LOCAL_CVE_CACHE`), lightweight summary list queries, explicit `gc.collect()`, `GET /health/memory` monitoring endpoint, and expanded 13-test automated verification suite (`test_agent_system.py`).
- [x] **Sentinel-AI Performance & UI Buffering Optimization (v1.2.0):** Implemented non-blocking async investigation pipeline (`POST /upload` immediate response), 6-stage SOC timeline progress metrics (`10% Parsing -> 35% CVE Intel -> 55% Risk Analysis -> 75% MITRE Mapping -> 90% Attack Graph -> 100% Report`), 44.5x status polling speedup via single-pass computed result caching, parallelized independent tool execution (`asyncio.gather`), route-based React lazy loading (`React.lazy` & `Suspense`), client-side resource caching (`RESOURCE_CACHE`), and component memoization (`React.memo`).
- [x] **Investigation Persistence & Supabase Restoration Fix (v1.2.1):** Fixed `user_id` forwarding regression in `agent_controller.py` `get_agent_status` that caused DB queries to return `None` (404) after cache eviction or browser refresh, added standardized `INVESTIGATION ID`, `USER ID`, `DATABASE ENGINE`, `SAVE START`, `SAVE SUCCESS`, `SAVE FAILURE` audit logs, sanitized JSON serialization in `SupabaseAdapter` and `SQLiteAdapter`, and verified persistence across all 13 production tests.
- [x] **Production Audit, Remediation Schema, Finding Correlation & MITRE Validation (v1.2.2):** Normalized remediation schema (`id`, `title`, `priority`, `why_it_matters`, `recommendation`, `mitre`, `cwe`, `difficulty`, `confidence`) with `"Not available"` UI fallbacks; implemented finding correlation layer merging multi-port exposures (SMB 139 & 445); upgraded LLM executive report to 3-paragraph SOC narrative; fixed Elasticsearch & database MITRE mappings with `validate_mitre_mapping`; added dynamic risk drivers and breakdown cards to `RiskDashboard.jsx`; updated labels to `"Attack Graph Entities"` and `Attacker Entry Point` → `Exposed Service` → `Vulnerability` → `MITRE Technique` → `Attack Objective`; expanded test suite to 17 automated tests.
- [x] **Investigation History Card Data & Title Fix (v1.2.3):** Updated `get_all_investigations` in `sqlite_adapter.py` and `supabase_adapter.py` to calculate lightweight `findings_count` without loading `full_state`; added dynamic scan title generation in `repository.py`; updated `DashboardOverview.jsx` card rendering for `findings_count` and title priority (`scan_name` -> `title` -> `user_goal` -> `"Security Investigation"`); added Test 18 to `test_agent_system.py`.
