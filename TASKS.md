...
- **Investigation Graph Visualization & Layer Separation (BUG-010):** Separated Technical Layer (`Asset → Service → Finding → CVE → MITRE → Remediation`) and Attack Layer (`Internet Exposure → Initial Access → Privilege Escalation → Lateral Movement → Data Exposure`), implemented compact node cards (`Name`, `Severity`, `Confidence`), host-grouped hierarchy layout, selective edge transition labeling, and enriched CVE nodes (`CVE ID`, `Severity`, `Impact`, `Confidence`).
